...
use mavlink::{MavConnection, MavHeader, MessageData};
...
fn main() -> Result<()> {
    ...
    let mut conn = mavlink::connect::<common::MavMessage>(&connect)
        .with_context(|| format!("failed to connect to {connect}"))?;
    conn.set_allow_recv_any_version(true);

    let identity = wait_for_heartbeat(&conn, Duration::from_secs(10))?;
    request_autopilot_version(&conn, &identity)?;
    let version = wait_for_autopilot_version(&conn, Duration::from_secs(3));
    ...
    request_params(&conn, &identity)?;
    let params = collect_params(&conn, Duration::from_secs(args.param_timeout))?;
    println!("received {} parameters", params.len());

    let sys_autostart = params.get("SYS_AUTOSTART").map(|p| p.value.round() as i32);
    let firmware_defaults = load_firmware_defaults(&args.px4_source, WATCHED_PARAMS)?;
    let airframe_defaults = if let Some(id) = sys_autostart {
...
fn wait_for_heartbeat(
    ...
    while Instant::now() < deadline {
        match conn.try_recv() {
            Ok((header, common::MavMessage::HEARTBEAT(heartbeat))) => {
...
fn request_autopilot_version(
    ...
    conn.send(&ground_station_header(), &msg)?;
    Ok(())
}
...
fn wait_for_autopilot_version(
    ...
    while Instant::now() < deadline {
        match conn.try_recv() {
            Ok((_, common::MavMessage::AUTOPILOT_VERSION(version))) => return Some(version),
            Ok(_) => {}
            Err(_) => thread::sleep(Duration::from_millis(20)),
        }
    }
...
fn collect_params(
    ...
    while Instant::now() < deadline {
        match conn.try_recv() {
            Ok((_, common::MavMessage::PARAM_VALUE(param))) => {
...
fn ground_station_header() -> MavHeader {
    MavHeader {
        system_id: 255,
        component_id: 190,
        sequence: 0,
...
fn load_airframe_defaults(
    ...
    Ok(parse_airframe_defaults(&text, &watched))
}
...
fn parse_airframe_defaults(text: &str, watched: &HashSet<&str>) -> HashMap<String, String> {
    let mut defaults = HashMap::new();
